...
const uri =
  "mongodb+srv://KIMITENLA:kimi1997@cluster0-f7nu1.gcp.mongodb.net/test?retryWrites=true&w=majority";
  ...
  if (method == "GET") {
    ...
    if (url == "/update_cuoc_hop") {
      ...
      Yeu_cau.on("end", function () {
        client.connect((uri) => {
          Xu_ly_3L.update_cuoc_hop(collection, Noi_dung_Nhan, db, client).then(
            (result) => {
              Dap_ung.writeHead(200, {
                "Content-Type": "Text/json; charset=utf-8",
              });
      ...
      Yeu_cau.on("end", function () {
        client.connect((uri) => {
          Xu_ly_3L.delete_cuoc_hop(collection, Noi_dung_Nhan, db, client).then(
            (result) => {
              Dap_ung.writeHead(200, {
                "Content-Type": "Text/json; charset=utf-8",
              });
...
Dich_vu.listen(Port, console.log(`Service Run: http://localhost:${Port}`));
